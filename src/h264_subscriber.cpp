...
void H264Subscriber::internalCallback(const sensor_msgs::CompressedImage::ConstPtr& message, const Callback& user_cb)
{
  ...
  convert_ctx =
      sws_getCachedContext(convert_ctx, picture_->width, picture_->height, AV_PIX_FMT_YUV420P, picture_->width,
                           picture_->height, AV_PIX_FMT_BGR24, SWS_FAST_BILINEAR, nullptr, nullptr, nullptr);
  out_->data.resize(picture_->width * picture_->height * 3);
  int stride = 3 * picture_->width;
  dst_ = out_->data.data();
  sws_scale(convert_ctx, picture_->data, picture_->linesize, 0, picture_->height, &dst_, &stride);
